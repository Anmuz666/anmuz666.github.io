...
export default {
  ...
  methods: {
    ...
    mapChart() {
      ...
      // $.getJSON(`/map/province/330000.json`, data => {
      // 	this.$echarts.registerMap('浙江', data);
      // 	initEcharts('浙江');
      // })
      initEcharts("china");
      ...
      myChart.on("click", param => {
        console.log(param);
        if (param.name in provincesList) {
          // 处理省模块
          console.log("加载省");
          let names = param.name;
          for (let key in provincesList) {
            if (names == key) {
              showProvince(provincesList[key], key);
              break;
            }
          }
        } else if (param.name in citysList) {
          // 处理市模块
          let names = param.name;
          console.log(names);
          for (let key in citysList) {
            if (names == key) {
              showCitys(citysList[key], key);
              break;
            }
          }
          ...
          let names = param.name;
          console.log(names);
          for (let key in Mapian) {
            if (names == key) {
              showC(Mapian[key], key);
              break;
            }
          }
        } else if (param.name in townList) {
          // 处理市模块
          let names = param.name;
          console.log(names);
          for (let key in townList) {
            if (names == key) {
              showTown(townList[key], key);
              break;
            }
          }
        }
      ...
      function showProvince(eName, param) {
        ...
        //axios方式
        axios
          .get(`/map/province/${eName}.json`)
          .then(res => {
            echarts.registerMap(param, res.data);
            initEcharts(param);
          })
          .catch(error => {
      ...
      function showTown(cName, param) {
        console.log(cName, param);
        axios
          .get(`/map/town/${cName}.json`)
          .then(res => {
            echarts.registerMap(param, res.data);
            initEcharts(param);
          })
